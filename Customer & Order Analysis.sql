-- In the following queries, I'm utilizing a database with multiple tables to quantify statistics regarding customer and order data after cleaning the orderID column. 
...
-- Shows how many of each type of headphone were sold in February.
SELECT product, 
...
-- Shows which product brought in the most revenue in January and what the total revenue was.
SELECT product, 
       Sum(quantity * price) AS revenue 
FROM   bit_db.jansales 
WHERE  Length(orderid) = 6 
